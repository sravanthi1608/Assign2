...
Given that 90\% of people are right-handed, let $ p = \frac{9}{10}$ be the probability of success (right-handed), and $ (1-p) =  \frac{1}{10}$ be the probability of failure (left-handed).
    ...
    \item We require $\pr{Y\le 6}$. Since $n = 10$,
             \begin{align}
                \pr{Y\le6} &= 1 - \pr{Y>6} \\
                           &= F_Y\brak{6}  \\
                           &= \sum_{k=0}^{6}p_Y(k) \\
                           &= \sum_{k=0}^{6}\myvec{n\\k}p^k\brak{1-p}^{n-k} \\
                           &= 0.0128
            \end{align}
\end{enumerate}  
\end{document}
